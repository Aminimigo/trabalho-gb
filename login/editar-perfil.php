...
// Se o formulário foi enviado, atualizar as informações no banco
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['foto_perfil']) && $_FILES['foto_perfil']['error'] === UPLOAD_ERR_OK) {
        ...
        $foto_destino = 'uploads/fotos_perfil/' . basename($foto_nome);

        if (move_uploaded_file($foto_tmp, $foto_destino)) {
            $foto_perfil = $foto_destino;
        } else {
            $foto_perfil = 'uploads/fotos_perfil/default-avatar.png'; // Caso não consiga fazer upload
        }
    } else {
        // Manter a foto atual se nenhuma nova foto for enviada
        $foto_perfil = $_POST['foto_atual'];
    }

    // Atualizar os dados no banco
    $stmt = $conn->prepare("UPDATE usuarios SET nome = :nome, telefone = :telefone, datanascimento = :datanascimento, foto_perfil = :foto_perfil, redes_sociais = :redes_sociais, portfolio = :portfolio WHERE email = :email");
    ...
    $stmt->bindParam(':redes_sociais', $redes_sociais);
    $stmt->bindParam(':portfolio', $portfolio);
    $stmt->bindParam(':email', $email_usuario);

    if ($stmt->execute()) {
        // Se a atualização for bem-sucedida, redirecionar para a página de visualização do perfil
...
// Recuperar os dados do usuário para exibir no formulário
...
$redes_sociais = isset($usuario['redes_sociais']) ? json_decode($usuario['redes_sociais'], true) : [];
            ...
            <form action="editar_perfil.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="field">
                    ...
                    <div class="control">
                        ...
                        <input type="hidden" name="foto_atual" value="<?php echo htmlspecialchars($foto_perfil); ?>">
                    </div>
